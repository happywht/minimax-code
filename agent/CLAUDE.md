...
- **2026-08-30** — v1.7.1（易用性专项：Preview per-project 根 + 实测修复，agent 侧）——① **`preview/server.py` re-root on switch**：`PreviewState.set_root(new_root)` 停旧 watcher → 换根 → 排空残留事件队列 → 仅当此前在跑才重启（同根 noop、stopped 保持 stopped）；`/preview/health`、`/preview/{path}`、`/preview/events` 三条路由闭包改请求时读 `state.workspace`（containment 守卫同随新根）；`build_app()` 把 HTTP 绑定的 state 发布为进程级单例（`set_preview_state`），stdio 模式 `ensure_preview_state()` 惰性补建；② **新 handler `ipc/handlers_preview.py`**：`preview.set_root {project_id?}`（省略/空白或项目未绑 `root_path` = 回落进程默认根，与 v1.3.0 workspace 语义一致；未知 project_id / 根目录不存在 → `-32602` 快速失败且 state 不动），IPC 方法 170→171、前缀 34→35；③ **缓存投毒修复**：`preview_file` 的 `FileResponse` 补 `Cache-Control: no-store`（实测：iframe 对同 URL 导航命中启发式缓存直接复用换根前旧文档，实时预览必须永远回源）；④ `version.py` `_FALLBACK_VERSION` 对齐发版（v1.7.0 曾漏 bump 残留 1.6.1，版本钉测试抓出）；+16 测试（`test_preview_server.py` +10 换根/路由跟随/单例 + `test_handlers_preview.py` 6 全新）
