"""Barrel surface regression baseline for ``minimax_code.memory`` (R237-R240).

Locks the public symbol set re-exported by ``minimax_code.memory`` so a future
leaf migration (R241+) must update both the count assertion and the set
...
Baseline (2026-07-22): 8 symbols = 2 classes (``SearchResult`` R237,
``Chunk`` R240) + 6 functions (``mmr_rerank`` R237, ``extract_keywords`` R238,
``has_markdown_headers`` + ``is_no_reply`` R239, ``chunk_hash`` +
``chunk_markdown`` R240).
``tokenize`` / ``jaccard_similarity`` (mmr), ``_STOP_WORDS`` (query_expansion),
and ``header_level`` (chunker; ``pub(crate)`` in grok) are module-public but
intentionally NOT barrel-exported -- they are leaf-internal helpers, mirroring
grok's private ``fn`` / ``static`` / ``pub(crate)`` visibility. ``text_utils``
has no module-private helpers -- both predicates are barrel-public.
"""
...
def test_package_barrel_exposes_eight_symbols() -> None:
    """R237 + R238 + R239 + R240 -- 8 symbols (2 classes + 6 fns).

    Future leaves must update count + set, or append a new migration-round
    group below.
    """
    assert len(memory.__all__) == 8
    assert set(memory.__all__) == {
        ...
        "is_no_reply",
        # R240 (chunker.py): markdown-aware semantic chunking.
        "Chunk",
        "chunk_hash",
        "chunk_markdown",
    }
