...
1. The 14 diagram-type tokens whose dedicated renderers ship in R289+ raise
   ...
   R288 (their dedicated dispatch arms no longer raise); the 14 remaining
   tokens still do.
...
def test_render_mermaid_to_svg_unsupported_type_raises(diagram_type: str) -> None:
    """Each of the 14 R289+ diagram tokens raises before the flowchart path.
    ...
    ``kanban`` renderer (R286), the ``timeline`` renderer (R287), and the
    ``quadrantChart`` renderer (R288) are asserted separately; none raises
