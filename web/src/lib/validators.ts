...
export function required(label = "This field"): Validator {
  return (v) => (v.trim() ? "" : `${label} is required`);
}

/** Minimum character length (after trim). */
export function minLength(min: number, label = "Value"): Validator {
  return (v) => (v.trim().length >= min ? "" : `${label} must be at least ${min} characters`);
}

/** Maximum character length. */
export function maxLength(max: number, label = "Value"): Validator {
  return (v) => (v.length <= max ? "" : `${label} must be at most ${max} characters`);
}
...
export function urlFormat(label = "URL"): Validator {
  return (v) => {
    ...
    try {
      const u = new URL(v);
      return u.protocol === "http:" || u.protocol === "https:" ? "" : `${label} must use http or https`;
    } catch {
...
export function regexFormat(label = "Pattern"): Validator {
  return (v) => {
    if (!v.trim()) return "";
    try {
      new RegExp(v);
      ...
      return `${label} is not a valid regex`;
    }
  };
}
