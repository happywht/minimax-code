...
export function SkeletonLine({ className }: SkeletonBaseProps): JSX.Element {
  return (
    <div
      className={`h-4 rounded bg-line animate-shimmer ${className ?? ""}`}
    />
  );
}
...
export function SkeletonCircle({ className }: SkeletonBaseProps): JSX.Element {
  return (
    <div
      className={`h-8 w-8 rounded-full bg-line animate-shimmer shrink-0 ${className ?? ""}`}
    />
  );
}
...
export function SkeletonCard({ className, lines = 2 }: SkeletonCardProps): JSX.Element {
  return (
    <div className={`rounded-lg border border-line bg-surface-2 p-4 space-y-3 ${className ?? ""}`}>
      <SkeletonLine className="h-5 w-2/3 bg-surface-3" />
  ...
  return (
    <div className={`space-y-2 ${className ?? ""}`}>
      {Array.from({ length: rows }, (_, i) => (
        <div key={i} className="flex items-center gap-3 rounded-md border border-line/40 bg-surface-2 px-3 py-2.5">
