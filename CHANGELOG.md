...
- **App 冒烟 shortcuts 用例并行 flake 加固**（v1.7.0 发版验证中发现）：`tests/app.test.tsx` 的 `?` 快捷键用例在 vitest 全量并行负载下间歇性红（单文件稳定过，失败率约四成）——两处竞态：前序用例 `user.type(textarea, ...)` 在 cleanup 卸载组件后 `document.activeElement` 残留指向已卸载的 textarea（App 的 `?` 处理器检查 activeElement 为 editable 即吞键），以及 keypress→setState→commit 链在全量负载下跨 act() 边界。加固：按键前显式 blur 归零焦点 + overlay 断言改 `waitFor`；修复后全量 6 连绿（诊断期经版本字符串对照实验排除发版 bump 因素，产品代码零改动）。
...
（v1.7.0 发版验证：pytest **10611 passed / 9 skipped / 0 failed**——单平台全量首次全绿（v1.6.1 备案的 7 项 Linux 平台差异红清零），安全回归 `-m security` 157 passed；vitest **782** 全绿（七轮累计：R1 +17 / R3 +3 / B3 +8 / B4 +3）；`tsc -b` 零错误、ESLint 零告警；e2e 11 spec——移动视口 6/6、桌面 smoke-boot 5/5；性能基线五项无 P0 退化（B2 复测：冷启动 median **2.138 s** 较 v1.6.1 锚点 2.291 s 的 −6.7%、首屏 JS 145.7 KB / CSS 8.3 KB gzip 预算内，判定无优化必要），数字随版固化于 `docs/performance-baseline.md`；发版时六处版本锚（pyproject / `__init__.py` / uv.lock 项目条目 / version.ts / 双 package.json）经 `test_version.py` 派生校验对齐。）
